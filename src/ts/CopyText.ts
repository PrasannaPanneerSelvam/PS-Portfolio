...
export default function CopyTextToClipBoard() {
  ...
  mailIdElement.addEventListener('click', () => {
    ...
    try {
      ...
      (mailIdElement.getElementsByClassName('mail')[0] as Maybe<HTMLElement>).dataset.status = 'Copied!';
      mailIdElement.classList.add('success');

      setTimeout(() => {
        mailIdElement.classList.remove('success');
        (mailIdElement.getElementsByClassName('mail')[0] as Maybe<HTMLElement>).dataset.status = 'Click to Copy';
      }, 1200);
    } catch (e) { }
  });
}